...
def read_config(config_name: str):
    ...
    mass_quest_teleport_key = parser.get(
        "hotkeys", "mass_quest_teleport", fallback="F6"
    )
    ...
    toggle_auto_combat_key = parser.get(
        "hotkeys", "toggle_auto_combat", fallback="NINE"
    )
    toggle_auto_dialogue_key = parser.get(
        "hotkeys", "toggle_auto_dialogue", fallback="F4"
    )
    ...
    toggle_auto_questing_key = parser.get(
        "hotkeys", "toggle_auto_questing", fallback="F3"
    )
    ...
    questing_friend_tp = parser.getboolean(
        "questing", "friend_teleport", fallback=False
    )
    gear_switching_in_solo_zones = parser.getboolean(
        "questing", "gear_switching_in_solo_zones", fallback=False
    )
    ...
    kill_minions_first = parser.getboolean(
        "combat", "kill_minions_first", fallback=False
    )
    automatic_team_based_combat = parser.getboolean(
        "combat", "automatic_team_based_combat", fallback=False
    )
    discard_duplicate_cards = parser.getboolean(
        "combat", "discard_duplicate_cards", fallback=True
    )

    # Auto Pet Settings
    global ignore_pet_level_up
    global only_play_dance_game
    ignore_pet_level_up = parser.getboolean(
        "auto pet", "ignore_pet_level_up", fallback=False
    )
    only_play_dance_game = parser.getboolean(
        "auto pet", "only_play_dance_game", fallback=False
    )
    ...
    for i in comparison_sections:
        ...
        for b in comparison_options:
            if not parser.has_option(i, b):
                print(
                    f'Config file section "{i}" lacks option "{b}", adding it and its default value.'
                )
    ...
    for i in sections:
        ...
        for b in options:
            if not comparison_parser.has_option(i, b):
                print(
                    f'Config file section "{i}" has erroneous option "{b}", removing it.'
                )
...
def hotkey_button(
    ...
    text_color: str = gui_text_color,
    button_color: str = gui_button_color,
):
    return gui.Button(
        name, button_color=(text_color, button_color), auto_size_button=auto_size
    )
...
async def navmap_teleport(
    ...
    if debug:
        if mass_teleport:
            logger.debug(
                f"{mass_quest_teleport_key} key pressed, teleporting all clients to quests."
            )
        else:
            logger.debug(
                f"{quest_teleport_key} key pressed, teleporting client {foreground_client.title} to quest."
            )
    clients_to_port = []
    if foreground_client:
...
async def friend_teleport_sync(clients: list[wizwalker.Client], debug: bool):
    ...
    if debug:
        logger.debug(
            f"{friend_teleport_key} key pressed, friend teleporting all clients to p1."
        )
...
async def tool_finish():
    ...
    await asyncio.gather(
        *[
            p.client_object.write_speed_multiplier(client_speeds[p.process_id])
...
async def main():
    ...
    async def toggle_speed_hotkey():
        ...
        if not freecam_status:
            if speed_task is not None and not speed_task.cancelled():
                ...
                speed_task = None
                logger.debug(
                    f"{toggle_speed_key} key pressed, disabling speed multiplier."
                )
                gui_send_queue.put(
                ...
                logger.debug(
                    f"{toggle_speed_key} key pressed, enabling speed multiplier."
                )
                gui_send_queue.put(
                    deimosgui.GUICommand(
                        deimosgui.GUICommandType.UpdateWindow,
    ...
    async def toggle_combat_hotkey(debug: bool = True):
        ...
        if not freecam_status:
            if combat_task is not None and not combat_task.cancelled():
                ...
                if debug:
                    logger.debug(
                        f"{toggle_auto_combat_key} key pressed, disabling auto combat."
                    )
                gui_send_queue.put(
                    deimosgui.GUICommand(
                        deimosgui.GUICommandType.UpdateWindow,
                ...
                if debug:
                    logger.debug(
                        f"{toggle_auto_combat_key} key pressed, enabling auto combat."
                    )
                gui_send_queue.put(
                    deimosgui.GUICommand(
                        deimosgui.GUICommandType.UpdateWindow,
    ...
    async def toggle_dialogue_hotkey(side_quests: bool = False):
        ...
        if not freecam_status:
            if dialogue_task is not None and not dialogue_task.cancelled():
                ...
                dialogue_task = None
                logger.debug(
                    f"{toggle_auto_dialogue_key} key pressed, disabling auto dialogue."
                )
                gui_send_queue.put(
                    deimosgui.GUICommand(
                        deimosgui.GUICommandType.UpdateWindow,
                ...
                logger.debug(
                    f"{toggle_auto_dialogue_key} key pressed, enabling auto dialogue{side_quest_log_str}."
                )
                gui_send_queue.put(
                    deimosgui.GUICommand(
                        deimosgui.GUICommandType.UpdateWindow,
    ...
    async def toggle_sigil_hotkey():
        ...
        if not freecam_status:
            ...
            if sigil_task is not None and not sigil_task.cancelled():
                ...
                sigil_task = None
                logger.debug(
                    f"{toggle_auto_sigil_key} key pressed, disabling auto sigil."
                )
                gui_send_queue.put(
                    deimosgui.GUICommand(
                        deimosgui.GUICommandType.UpdateWindow,
                ...
                logger.debug(
                    f"{toggle_auto_sigil_key} key pressed, enabling auto sigil."
                )
                if questing_task is not None and not questing_task.cancelled():
                    logger.debug(
                        f"{toggle_auto_questing_key} key pressed, disabling auto questing."
                    )
                    gui_send_queue.put(
                        deimosgui.GUICommand(
                            deimosgui.GUICommandType.UpdateWindow,
        ...
        if foreground_client:
            if await is_free(foreground_client):
                if await foreground_client.game_client.is_freecam():
                    if debug:
                        logger.debug(
                            f"{toggle_freecam_key} key pressed, disabling freecam."
                        )
                    ...
                    if debug:
                        logger.debug(
                            f"{toggle_freecam_key} key pressed, enabling freecam."
                        )
    ...
    async def tp_to_freecam_hotkey():
        if foreground_client:
            logger.debug(
                f"Shift + {toggle_freecam_key} key pressed, teleporting foreground client to freecam position."
            )
            if await foreground_client.game_client.is_freecam():
    ...
    async def toggle_questing_hotkey():
        ...
        if not freecam_status:
            ...
            if questing_task is not None and not questing_task.cancelled():
                ...
                logger.debug(
                    f"{toggle_auto_questing_key} key pressed, enabling auto questing."
                )
                gui_send_queue.put(
                    deimosgui.GUICommand(
                        deimosgui.GUICommandType.UpdateWindow,
    ...
    async def enable_hotkeys(exclude_freecam: bool = False, debug: bool = False):
        ...
        if not hotkey_status:
            ...
            await listener.add_hotkey(
                Keycode[x_press_key], x_press_hotkey, modifiers=ModifierKeys.NOREPEAT
            )
            # await listener.add_hotkey(Keycode[space_press_key], space_press_hotkey, modifiers=ModifierKeys.NOREPEAT)
            await listener.add_hotkey(
                Keycode[sync_locations_key],
                xyz_sync_hotkey,
                modifiers=ModifierKeys.NOREPEAT,
            )
            await listener.add_hotkey(
                Keycode[quest_teleport_key],
                navmap_teleport_hotkey,
                modifiers=ModifierKeys.NOREPEAT,
            )
            await listener.add_hotkey(
                Keycode[mass_quest_teleport_key],
                mass_navmap_teleport_hotkey,
                modifiers=ModifierKeys.NOREPEAT,
            )
            await listener.add_hotkey(
                Keycode[toggle_speed_key],
                toggle_speed_hotkey,
                modifiers=ModifierKeys.NOREPEAT,
            )
            await listener.add_hotkey(
                Keycode[friend_teleport_key],
                friend_teleport_sync_hotkey,
                modifiers=ModifierKeys.NOREPEAT,
            )
            await listener.add_hotkey(
                Keycode[toggle_auto_combat_key],
                toggle_combat_hotkey,
                modifiers=ModifierKeys.NOREPEAT,
            )
            await listener.add_hotkey(
                Keycode[toggle_auto_fish_key],
                toggle_auto_fish_hotkey,
                modifiers=ModifierKeys.NOREPEAT,
            )
            await listener.add_hotkey(
                Keycode[toggle_auto_dialogue_key],
                toggle_dialogue_hotkey,
                modifiers=ModifierKeys.NOREPEAT,
            )
            await listener.add_hotkey(
                Keycode[toggle_auto_dialogue_key],
                toggle_dialogue_side_quests_hotkey,
                modifiers=ModifierKeys.SHIFT | ModifierKeys.NOREPEAT,
            )
            await listener.add_hotkey(
                Keycode[toggle_auto_sigil_key],
                toggle_sigil_hotkey,
                modifiers=ModifierKeys.NOREPEAT,
            )
            if not exclude_freecam:
                await listener.add_hotkey(
                    Keycode[toggle_freecam_key],
                    toggle_freecam_hotkey,
                    modifiers=ModifierKeys.NOREPEAT,
                )
                await listener.add_hotkey(
                    Keycode[toggle_freecam_key],
                    tp_to_freecam_hotkey,
                    modifiers=ModifierKeys.SHIFT | ModifierKeys.NOREPEAT,
                )
            await listener.add_hotkey(
                Keycode[toggle_auto_questing_key],
                toggle_questing_hotkey,
                modifiers=ModifierKeys.NOREPEAT,
            )
            hotkey_status = True
    ...
    async def disable_hotkeys(
        ...
        if hotkey_status:
            ...
            await listener.remove_hotkey(
                Keycode[x_press_key], modifiers=ModifierKeys.NOREPEAT
            )
            # await listener.remove_hotkey(Keycode[space_press_key], modifiers=ModifierKeys.NOREPEAT)
            await listener.remove_hotkey(
                Keycode[sync_locations_key], modifiers=ModifierKeys.NOREPEAT
            )
            await listener.remove_hotkey(
                Keycode[quest_teleport_key], modifiers=ModifierKeys.NOREPEAT
            )
            await listener.remove_hotkey(
                Keycode[mass_quest_teleport_key], modifiers=ModifierKeys.NOREPEAT
            )
            await listener.remove_hotkey(
                Keycode[toggle_speed_key], modifiers=ModifierKeys.NOREPEAT
            )
            await listener.remove_hotkey(
                Keycode[friend_teleport_key], modifiers=ModifierKeys.NOREPEAT
            )
            if not exclude_kill:
                await listener.remove_hotkey(
                    Keycode[kill_tool_key], modifiers=ModifierKeys.NOREPEAT
                )
            await listener.remove_hotkey(
                Keycode[toggle_auto_combat_key], modifiers=ModifierKeys.NOREPEAT
            )
            await listener.remove_hotkey(
                Keycode[toggle_auto_fish_key], modifiers=ModifierKeys.NOREPEAT
            )
            await listener.remove_hotkey(
                Keycode[toggle_auto_dialogue_key], modifiers=ModifierKeys.NOREPEAT
            )
            await listener.remove_hotkey(
                Keycode[toggle_auto_dialogue_key],
                modifiers=ModifierKeys.SHIFT | ModifierKeys.NOREPEAT,
            ...
            await listener.remove_hotkey(
                Keycode[toggle_auto_sigil_key], modifiers=ModifierKeys.NOREPEAT
            )
            if not exclude_freecam:
                await listener.remove_hotkey(
                    Keycode[toggle_freecam_key], modifiers=ModifierKeys.NOREPEAT
                )
                await listener.remove_hotkey(
                    Keycode[toggle_freecam_key],
                    modifiers=ModifierKeys.SHIFT | ModifierKeys.NOREPEAT,
            ...
            await listener.remove_hotkey(
                Keycode[toggle_auto_questing_key], modifiers=ModifierKeys.NOREPEAT
            )
            hotkey_status = False
        ...
        while True:
                    ...
                    match com.com_type:
                                ...
                                case GUIKeys.copy_entity_list:
                                    if foreground_client:
                                        ...
                                        if entities_info:
                                            ...
                                            logger.error(
                                                "Failed to load Entity list. Please try again."
                                            )
                                ...
                                case GUIKeys.copy_ui_tree:
                                    if foreground_client:
                                        ...
                                        async def get_ui_tree(
                                            ...
                                            for child in await utils.wait_for_non_error(
                                                window.children
                                            ):
                        ...
                        case deimosgui.GUICommandType.EntityTeleport:
                            ...
                            if foreground_client:
                                sprinter = SprintyClient(foreground_client)
                                entities = (
                                    await sprinter.get_base_entities_with_vague_name(
                                        com.data
                                    )
                                )
                                if entities:
                                    entity = await sprinter.find_closest_of_entities(
                                        entities
                                    )
                                    entity_pos = await entity.location()
                            ...
                            if (
                                ...
                                gui_send_queue.put(
                                    deimosgui.GUICommand(
                                        ...
                                        ("EnemyInput", names_list),
                                    )
                                )
                                gui_send_queue.put(
                                    deimosgui.GUICommand(
                                        deimosgui.GUICommandType.UpdateWindowValues,
                                        ("AllyInput", names_list),
                                    )
                                )
                                gui_send_queue.put(
                                    deimosgui.GUICommand(
                                        deimosgui.GUICommandType.UpdateWindow,
                                        ("EnemyInput", names_list[caster_i]),
                                    )
                                )
                                gui_send_queue.put(
                                    deimosgui.GUICommand(
                                        deimosgui.GUICommandType.UpdateWindow,
                                        ("AllyInput", names_list[target_i]),
                                    )
                                )
                                gui_send_queue.put(
                                    deimosgui.GUICommand(
                                        deimosgui.GUICommandType.UpdateWindow,
                                        ("SchoolInput", school_name),
                                    )
                                )
                            else:
                                logger.info(
                                    "Last selected client is not currently in combat. You must be in combat to use the stat viewer."
                                )
            ...
            if walker.clients:
                ...
                gui_send_queue.put(
                    deimosgui.GUICommand(
                        deimosgui.GUICommandType.UpdateWindow,
                        ("Auto FishStatus", bool_to_string(is_fishing)),
                    )
                )
            await asyncio.sleep(0.1)
            ...
            # 	pass
                ...
                while True:
                    ...
                    try:
                        ...
                        await rpc.update(
                            state=f"{task_str}In {status_str}{end_zone}",
        ...
        if not default_logic:
            ...
            try:
                ...
                while walker.clients:
                    ...
                    await asyncio.sleep(0.1)
                logger.debug(
                    "Waiting for all previous Wizard101 clients to be reopened..."
                )
    ...
    await listener.add_hotkey(
        Keycode[kill_tool_key], kill_tool_hotkey, modifiers=ModifierKeys.NOREPEAT
    )
    ...
    try:
        foreground_client_switching_task = asyncio.create_task(
            foreground_client_switching()
        )
        assign_foreground_clients_task = asyncio.create_task(
        ...
        questing_leader_combat_detection_task = asyncio.create_task(
