...
def build_launcher_tab(ctx):
    ...
    def _text_qcolor() -> QColor:
        text_color = getattr(ctx, "text_color", None)

        if not text_color:
            text_color = (
                "#ffffff"
                if getattr(ctx, "theme", "dark") in ("black", "dark")
                else "#000000"
            )

        qcolor = QColor(str(text_color))
    ...
    def _text_rgba(alpha: int) -> str:
        qcolor = _text_qcolor()
        return f"rgba({qcolor.red()},{qcolor.green()},{qcolor.blue()},{alpha})"

    def _muted_text_style(alpha: int) -> str:
        return f"color: {_text_rgba(alpha)};"

    def _launcher_list_style() -> str:
        hover_rgba = _text_rgba(15)
        scroll_rgba = _text_rgba(40)
        text_color = getattr(ctx, "text_color", None)
        ...
        return (
            ...
            f"QScrollBar::handle:vertical {{ background: {scroll_rgba}; border-radius: 3px; min-height: 20px; }}"
    ...
    def _build_account_item_widget(nickname: str, disabled: bool = False):
        ...
        if disabled:
            cb.setEnabled(False)
            cb.setChecked(False)
            lbl.setStyleSheet(_muted_text_style(80))
    ...
    def _build_unmanaged_client_widget(handle: int):
        ...
        lbl.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
        lbl.setStyleSheet(_muted_text_style(120))
        row_layout.addWidget(lbl, 1)
    ...
    def _rebuild_hooked_clients_list():
        ...
        if remaining:
            ...
            sep_lbl.setStyleSheet(_muted_text_style(80) + " font-size: 10px;")
    ...
    def _launch_and_login():
        ...
        if selected:
            ...
            send_queue.put(
                GUICommand(GUICommandType.LaunchInstance, (selected, game_path))
            )
    ...
    def _show_settings_dialog():
        ...
        path_row.addWidget(
            launcher_icon_btn(ctx, svgs["folder"], tl("game_path"), _pick)
        )
    ...
    launcher_action_row.addWidget(
        ctx.registry.action_icon_btn(
            svgs["gear"], tl("settings"), _show_settings_dialog
        )
    )
    launcher_action_row.addStretch()
    ...
    def _retheme_launcher():
        account_list.setStyleSheet(_launcher_list_style())
        hooked_clients_list.setStyleSheet(_launcher_list_style())

        managed_accounts = ctx.widget_tags.get("managed_accounts", set())
