...
from wizwalker.extensions.wizsprinter import CombatConfigProvider, SprintyCombat
from wizwalker.extensions.wizsprinter.combat_backends.backend_base import (
    BaseCombatBackend,
)
from wizwalker.extensions.wizsprinter.combat_backends.combat_api import (
    CombatConfig,
    SpellType,
    TargetType,
    TemplateSpell,
)
from wizwalker.extensions.wizsprinter.combat_backends.config_backend import (
    Lark,
    TreeToConfig,
    get_sprinty_grammar,
)
...
class StrCombatConfigProvider(CombatConfigProvider):
    ...
    def __init__(self, config_data: str = "", cast_time: float = 0.2):
        super(CombatConfigProvider, self).__init__(
            cast_time
        )  # Bypass init function of parent
        self.filename = "Config"
        self.config: CombatConfig = self.parse_config(config_data)

    async def handle_no_cards_given(self):
        raise RuntimeError(
            "Full config fail! Config might be empty or contains only explicit rounds. Consider adding a pass or something else."
        )
...
def delegate_combat_configs(
    input_data: str, fallback_clients: int = 1, line_seperator: str = "\n"
) -> Dict[int, str]:
    """
